...
local NFREQ = 1000
--local FREQ = linspace(0,fmax,NFREQ)
local FREQ = matrix(range(0,NFREQ):map(function(i) return i/NFREQ * fmax end))
...
local nz_src = 2						-- where to inject the source
										-- insert at the point just past where we are reading the reflection
...
local _i = complex(0,1)
local K = FREQ:map(function(x) return (-_i*2*math.pi*dt*x):exp() end)
local REF = matrix{NFREQ}:zeros()
local TRN = matrix{NFREQ}:zeros()
local SRC = matrix{NFREQ}:zeros()
...
local loop = (function()
	...
	for T = 1,STEPS do
		...
		for nf=1,NFREQ do
error"why isn't ref normalized + trn normalized equal to 1?"			
			-- integrating electric field at each point, over time
			REF[nf] = REF[nf] + (K[nf]^T)*Ey[1]		-- reflectance
			TRN[nf] = TRN[nf] + (K[nf]^T)*Ey[Nz]	-- transmittance
			SRC[nf] = SRC[nf] + (K[nf]^T)*Esrc[T]	-- source
		end
...
App.title = 'FTDT test'
function App:update()
	-- noramlize fourier transform to source
	-- this goes last in the example
	local refn = range(#REF):map(function(i) 
		local x = REF[i] / SRC[i]
		x = complex.abs(x)
		if type(x) ~= 'number' then x = x.re end
		return x^2
	end)
	local trnn = range(#TRN):map(function(i)
		local x = TRN[i] / SRC[i]
		x = complex.abs(x)
		if type(x) ~= 'number' then x = x.re end
		return x^2
	end)
	local conn = range(#TRN):map(function(i) return refn[i] + trnn[i] end)

	local graphs = table{
		{buf=ER, color={1,0,0}},
		{buf=Ey, color={1,1,0}},
		{buf=Hx, color={0,1,1}},
		{buf=refn, color={1,0,1}},
		{buf=trnn, color={1,.5,0}},
		{buf=conn, color={1,.5,0}},
	}
	...
	for _,graph in ipairs(graphs) do
		...
		gl.glColor3f(table.unpack(graph.color))
		for nz=1,Nz do
			gl.glVertex2f(za[nz], graph.buf[nz])
		end
		gl.glEnd()
	end
